                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tr v-for="row in gridData">
                            ...
                            <td >
                                ...
                                <button class="btn btn-success" @click.prevent="confirmActivate(row.Id)">開通</button>
...
<script>
    export default {
        ...
        methods: {
            ...
            loadData: function (){
                $.getJSON('/api/backend-unactivated-users-data', function (data) {
                    this.gridData = data;
            ...
            getData: function (request){
                ...
                switch (request){
                    ...
                    //     break;
                    case this.query :
                        getPage = '/api/backend-unactivated-users-data?' +
                                  'keyword=' + this.query +
                                  '&column=' + this.sortKey +
                                  '&direction=' + this.sortOrder;
                        break;
            ...
            activateItem: function (id) {
                ...
                this.$http.get('/backend/users/'+id+'/manual-activate' ).then((response) => {
                    this.getData();
                });
            },
            confirmActivate: function(id)
            {
                var x = confirm("確定開通此使用者? 此更新不可逆");
                if (x)
